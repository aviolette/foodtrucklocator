var TruckScheduleWidget = function () {
  ...
  function invokeEditDialog(stop, afterwards) {
    ...
    $("#truck-schedule-alert").addClass("d-none");
      ...
      if (locationName.length == 0) {
        ...
        $.ajax({
          ...
          error: function (e) {
            ...
            $("#truck-schedule-alert").removeClass("d-none");
  ...
  function refreshSchedule() {
    ...
    var timeQuery = "";
    var calendarQuery = $("#calendarListTable").hasClass("d-none");
    if (calendarQuery) {
    ...
    $.each(schedule, function (truckIndex, stop) {
      ...
      if (showControls) {
        if (!prevHadStart && now < stop.startMillis) {
          prevHadStart = true;
          buf = buf + "<button class='btn btn-outline-info' id='truckStartNow" + truckIndex +
              "' class='btn success'>Start</button>"
        } else if (now >= stop.startMillis && now < stop.endMillis) {
          buf = buf + "<button class='btn btn-outline-warning' id='truckEndNow" + truckIndex +
              "' class='btn btn-outline-warning'>End</button>";
        }
      }
      buf += "&nbsp;</td><td>";
      if (showControls) {
        buf = buf + "<div class='btn-group'><button class='btn btn-outline-info' id='truckDelete" + truckIndex +
            "' class='btn '><span class='icon icon-trash'></span> </button>&nbsp;<button class='btn btn-outline-info' id='truckEdit" +
            truckIndex + "'><span class='icon icon-pencil'></span> </button></div></td></tr>";
      }
  ...
  return {
    ...
    init: function (truckId, locations, categories, options) {
      ...
      if (_options["vendorEndpoints"]) {
        ...
        $("#delay-warning").addClass("d-none");
        $("#truck-widget-header").addClass("d-none");
      }
      ...
      $("#scheduleCalendarButton").click(function () {
        $("#calendarListTable").addClass("d-none");
        $("#calendarTable").removeClass("d-none");
        $(this).removeClass("btn-default");
        $("#scheduleListButton")
            .addClass("btn-outline-secondary")
            .removeClass("btn-outline-primary");
        $("#scheduleCalendarButton").removeClass("btn-outline-secondary").addClass("btn-outline-primary");
        if (!_hasFullSchedule) {
          refreshSchedule();
        }
      });
      ...
      $("#scheduleListButton").click(function () {
        $("#calendarListTable").removeClass("d-none");
        $("#calendarTable").addClass("d-none");
        $(this).removeClass("btn-default");
        $("#scheduleCalendarButton").addClass("btn-outline-secondary").removeClass("btn-outline-primary");
        $("#scheduleListButton").removeClass("btn-outline-secondary").addClass("btn-outline-primary");
      });
