...
<script type="text/javascript">
  ...
  (function () {
    TruckScheduleWidget.init("${truck.id}", ${locations},
        [<c:forEach var="category" varStatus="categoryIndex" items="${truck.categories}">"${category}"<c:if test="${!categoryIndex.last}">, </c:if></c:forEach>], {hasCalendar: ${not empty(truck.calendarUrl) || not empty(truck.drupalCalendar) || not empty(truck.icalCalendar) || truck.id == "thefatshallot" || not empty(truck.squarespaceCalendar)}});
