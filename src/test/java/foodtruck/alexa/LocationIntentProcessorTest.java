...
public class LocationIntentProcessorTest extends EasyMockSupport {
  ...
  public void before() {
    ...
    DailyScheduleWriter dailyScheduleWriter = createMock(DailyScheduleWriter.class);
  ...
  public void processWithNoDateNoTruckUnresolved() throws Exception {
    Intent intent = Intent.builder()
        ...
        .withSlots(ImmutableMap.of(SLOT_LOCATION, Slot.builder()
            ...
            .build()))
        .build();
    expect(locator.locate("Clark and Monroe", GeolocationGranularity.NARROW)).andReturn(null);
    expect(cacher.findSchedule()).andReturn(DAILY_SCHEDULE);
    dailyScheduleNullLocations();
    expect(locationDAO.findById(28065L)).andReturn(null);
    expect(locationDAO.findById(5732531110412288L)).andReturn(null);
  ...
  public void processWithNoDateNoTruckUnresolvedAlternates() throws Exception {
    ...
    expect(cacher.findSchedule()).andReturn(DAILY_SCHEDULE);
    dailyScheduleNullLocations();
    expect(locationDAO.findById(28065L)).andReturn(Location.builder()
        .lat(12)
        .lng(13.01)
        .name("Foos")
        .build());
    expect(locationDAO.findById(5732531110412288L)).andReturn(Location.builder()
        .lat(12)
        .lng(13.011)
        .name("Hoos")
        .build());
    expect(locationDAO.findById(50018L)).andReturn(Location.builder()
        .lat(40)
        .lng(-88.01)
        .name("Blah")
        .build());
    expect(locationDAO.findById(1835344L)).andReturn(Location.builder()
        .lat(12.01)
        .lng(13.0)
        .name("Foobar")
        .build());
  ...
  private void dailyScheduleNullLocations() {
    expect(locationDAO.findById(2174002L)).andReturn(null);
    expect(locationDAO.findById(501097L)).andReturn(null);
    expect(locationDAO.findById(555287L)).andReturn(null);
    expect(locationDAO.findById(51101L)).andReturn(null);
    expect(locationDAO.findById(6259051008622592L)).andReturn(null);
    expect(locationDAO.findById(2231491L)).andReturn(null);
    expect(locationDAO.findById(5810612923793408L)).andReturn(null);
  }
