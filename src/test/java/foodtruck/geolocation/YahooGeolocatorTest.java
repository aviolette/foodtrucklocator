...
public class YahooGeolocatorTest extends EasyMockSupport {
  ...
  public void testCityResultShouldReturnNull() throws Exception {
    ...
    expect(resource.findLocation("Beer and Wine, Chicago, IL", false)).andReturn(returnCode);
    replayAll();
  ...
  public void testIntersectionShouldReturnResult() throws Exception {
    ...
    expect(resource.findLocation("Dearborn and Monroe, Chicago, IL", false)).andReturn(returnCode);
    replayAll();
    Location location = yahooGeolocator.locate("Dearborn and Monroe, Chicago, IL",
        GeolocationGranularity.BROAD);
    assertEquals(
        Location.builder().lat(41.880730).lng(-87.629379).name("Dearborn and Monroe, Chicago, IL")
            .build(), location);
    verifyAll();
  }
