...
public class AmanecerTacosMatcher extends AbstractSpecialMatcher {
  ...
  @Inject
  public AmanecerTacosMatcher(GeoLocator geoLocator) {
    super(geoLocator, ImmutableList.of(
        ...
        new Spot("davis station", "Davis Station")));
  }
  ...
  public void handle(TruckStopMatch.Builder builder, Story story, Truck truck) {
    ...
    if (m.find()) {
      matchAt(story.getTime().withTime(8, 0, 0, 0), story.getTime().withTime(8, 45, 0, 0), m.group(1), stops, truck);
      matchAt(story.getTime().withTime(9, 0, 0, 0), story.getTime().withTime(9, 45, 0, 0), m.group(2), stops, truck);
      matchAt(story.getTime().withTime(10, 0, 0, 0), story.getTime().withTime(10, 45, 0, 0), m.group(3), stops, truck);
    }
  ...
  private void matchAt(DateTime startTime, DateTime endTime, String text, ImmutableList.Builder<TruckStop> stops,
    ...
    stops.add(TruckStop.builder().origin(StopOrigin.TWITTER).truck(truck).locked(true)
        .location(location)
        .startTime(startTime)
        .endTime(endTime)
        .build());
